      ...
      <div class="group relative rounded-2xl bg-slate-900 border border-slate-800 hover:border-indigo-500/50 p-2.5 transition-all duration-200 hover:scale-[1.02] flex flex-col justify-between shadow-lg hover:shadow-indigo-500/10">
        ...
        <div class="relative w-full aspect-[2/3] rounded-xl overflow-hidden bg-slate-950 border border-slate-800/60 flex items-center justify-center">
          ...
          <img
            src="${poster}"
            alt="${title}"
            class="w-full h-full object-cover group-hover:opacity-95 transition-opacity relative z-0"
            loading="lazy"
          ...
          <div class="absolute inset-0 bg-slate-950/85 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-3 text-center z-20 pointer-events-none group-hover:pointer-events-auto">
            ...
            <a
              href="${stremioHref}"
              class="w-full inline-flex items-center justify-center gap-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-200 transition-colors"
            >
            ...
            <button
              ...
              data-id="${item.id}"
              data-title="${title}"
              class="w-full inline-flex items-center justify-center gap-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-300 hover:text-rose-200 px-3 py-1.5 text-[11px] font-semibold transition-colors mt-0.5"
            >
    ...
    return `
      <a href="${detailUrl}" class="group relative rounded-xl bg-slate-900 border border-slate-800 hover:border-indigo-500/50 p-2 transition-all hover:scale-[1.02] flex flex-col justify-between shadow-md">
        ...
        <div class="relative w-full aspect-[2/3] rounded-lg overflow-hidden bg-slate-950 flex items-center justify-center">
          ...
          <img src="${poster}" alt="${displayTitle}" class="w-full h-full object-cover group-hover:opacity-90 transition-opacity relative z-0" loading="lazy" onload="this.previousElementSibling && this.previousElementSibling.remove()" onerror="this.src='${PLACEHOLDER_POSTER}'; this.previousElementSibling && this.previousElementSibling.remove()">
          
          <!-- Top Badges -->
          <div class="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded bg-black/75 backdrop-blur text-[10px] font-bold text-slate-200 z-10">
            ${epBadge}
          </div>

          <div class="absolute top-1.5 right-1.5 flex items-center gap-1 px-1.5 py-0.5 rounded bg-black/75 backdrop-blur text-[10px] font-semibold text-slate-300 z-10">
            ${statusDot}
